...
import { SabActor } from "./documents/actor.mjs";
import { SabItem } from "./documents/item.mjs";
...
Hooks.once("init", function() {
  ...
  CONFIG.Item.dataModels = {
    item: models.SabItem,
    feature: models.SabFeature,
    spell: models.SabSpell
  };
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.spellburnandbattlescars.rollItemMacro("${data.uuid}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { "spellburn-and-battlescars.itemMacro": true }
    });
  }
